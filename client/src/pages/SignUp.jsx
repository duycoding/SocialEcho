...
const SignUpNew = () => {
	...
	const handleAvatarChange = (e) => {
		...
		if (
			...
			setAvatar(null);
			setAvatarError(
				'Vui lòng sử dụng một trong các định dạng ảnh (jpeg, jpg, png)',
			);
		} else if (file.size > 10 * 1024 * 1024) {
			setAvatar(null);
			setAvatarError('Vui lòng tải hình ảnh có dung lượng nhỏ hơn 10MB');
		} else {
			setAvatar(file);
			setAvatarError(null);
		}
	...
	const handleSubmit = async (e) => {
		...
		const timeout = setTimeout(() => {
			setLoadingText(
				'Việc này tốn nhiều thời gian hơn bình thường. Vui lòng đợi trong khi các dịch vụ phụ trợ đang bắt đầu.',
			);
		}, 5000);
			...
			<div className='container mx-auto flex min-h-screen items-center justify-center px-6'>
				<form
					...
					<label
						...
						<h2 className='mx-3 text-gray-400'>Ảnh đại diện</h2>
						<input
							id='avatar'
							type='file'
							className='hidden'
						...
						<div
							...
							{isConsentGiven && !isModerator ? (
								<p className='mt-2 cursor-pointer rounded-lg border border-green-500 px-4 py-3 text-center text-sm font-semibold text-green-600'>
									Xác thực dựa trên ngữ cảnh đã được kích hoạt
								</p>
							) : (
								<p className='mt-2 cursor-pointer rounded-lg border px-4 py-3 text-center text-sm font-semibold'>
									Xác thực dựa trên ngữ cảnh đã chưa được kích hoạt
								</p>
							)}
						</div>
